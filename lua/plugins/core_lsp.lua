...
local on_attach = function(_, bufnr)
    ...
    nmap('<leader>dl', '<cmd>Lspsaga show_buf_diagnostics<CR>', '[D]i[A]gnostics [L]ist')
    -- nmap('<leader>dl', builtin.diagnostics, '[D]i[A]gnostics [L]ist')

    -- nmap('<leader>ca', vim.lsp.buf.code_action, '[C]ode [A]ction')
    nmap('<leader>ca', '<cmd>Lspsaga code_action<cr>', '[C]ode [A]ction')

    nmap('<leader>fm', function() vim.lsp.buf.format { async = true } end, '[C] [F]or[m]at code')

    -- nmap('<leader>ot', "<cmd>Lspsaga outline<CR>", '[O]u[T] line')
    nmap('<leader>ot', builtin.lsp_document_symbols, '[O]u[T] line')

    nmap('gk', '<cmd>Lspsaga peek_definition<cr>', '[C] [G]o to [P]eek definition')

    vim.keymap.set('n', '<leader>ft', '<cmd>Lspsaga term_toggle<cr>', { desc = '[C] Open [f]loat [t]erminal' })
end
